...
import { updateFieldInPageDraft, updateFieldInGlobalsDraft } from '@/lib/cms-local-changes';
...
export function CommitViewer({ commitSha }: CommitViewerProps) {
  ...
  const handleRecoverField = useCallback(
    async (info: RecoverFieldInfo): Promise<boolean> => {
      ...
      try {
        ...
        if (pageId === 'globals') {
          ...
          success = await updateFieldInGlobalsDraft(
            componentId,
            fieldName,
            valueToRecover,
            locale,
          ...
          success = await updateFieldInPageDraft(
            pageId,
            componentId,
            fieldName,
            valueToRecover,
      ...
      try {
        ...
        const fileChangePromises = contentFiles.map(async (file) => {
          ...
          try {
            ...
            const oldRawData =
              details.parentSha && file.status !== 'added'
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-muted-foreground/60 bg-muted/5">
        ...
        <p className="text-sm mt-1 max-w-xs text-center">
          Select a commit from the history sidebar to view details
        </p>
      </div>
    );
  }
    ...
    <div className="flex h-[calc(100vh-41px)] overflow-hidden bg-background">
      ...
      <div className="w-[300px] flex-shrink-0 border-r flex flex-col bg-muted/10">
        ...
        <div className="px-4 py-2 text-xs font-semibold text-muted-foreground bg-muted/5 flex items-center justify-between border-b">
          ...
          <Badge
            variant="secondary"
            className="h-5 px-1.5 text-[10px] min-w-[1.25rem] border border-border dark:border-none justify-center"
          >
            {fileChanges.length}
          </Badge>
        </div>

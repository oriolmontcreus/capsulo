...
interface RenderingProps {
  ...
  calculateVisibleRange: (items: SelectOption[]) => {
    start: number;
    end: number;
    items: SelectOption[];
  };
...
export const useRendering = (props: RenderingProps) => {
  ...
  const renderOptionContent = (opt: any, isSelected: boolean = false) => {
    ...
    return (
      <div className="flex items-center gap-2 w-full">
        {hasOptPrefix && <span className="flex shrink-0 items-center">{opt.prefix}</span>}
        ...
        {hasOptSuffix && <span className="flex shrink-0 items-center">{opt.suffix}</span>}
      </div>
    );
  ...
  const renderSingleCommandItem = (opt: SelectOption, virtualIndex?: number) => {
    const queryToUse =
      field.minSearchLength && debouncedSearchQuery.length < field.minSearchLength
        ? ''
        : debouncedSearchQuery;

...
- **Naming Convention**: `{SchemaName}Data`
  - `HeroSchema` → `HeroSchemaData`
  - `Card` (Repeater item) → `CardData`
...
- ✅ Input, Textarea, RichEditor
- ✅ ColorPicker, Select, FileUpload
- ✅ Switch (boolean)
- ✅ DateField (Date)
- ✅ Repeater (with nested types)

### Modifiers

- ✅ `.required()` - Makes field non-optional
- ✅ `.defaultValue()` - Makes field non-optional
- ✅ `.itemName()` - Names Repeater item interface
- ✅ `.tab()` - Extracts fields from tabs
- ✅ `.contains()` - Extracts fields from grids

### Layout Components

- ✅ Tabs - Fields are extracted from tab arrays
- ✅ Grid - Fields are extracted from contains arrays

## Limitations

- Only processes files with `.schema.tsx` extension
- Assumes schema variable names end with `Schema` or use `createSchema()`
- Astro component must have the same base name as the schema (e.g., `HeroSchema` → `Hero.astro`)
...
## Automatic Type Generation (Dev Mode)

As of the latest update, schema types are **automatically regenerated during development mode**. This feature is implemented via a Vite plugin that watches for changes to `.schema.tsx` files.

### How It Works

When you run `npm run dev`, the `vite-plugin-schema-types.ts` plugin:

1. **Initial Scan**: On dev server start, scans all `.schema.tsx` files and regenerates any stale `.d.ts` files
2. **File Watching**: Monitors all `.schema.tsx` files for changes
3. **Debounced Regeneration**: When a schema changes, waits 100ms (debounce) then regenerates the corresponding `.d.ts` file
4. **Change Detection**: Only writes to disk if the content actually changed
5. **Error Handling**: Logs errors without crashing the dev server
...
```
[schema-types] Regenerated: hero.schema.d.ts
```

### Gitignore Configuration

If you prefer not to commit auto-generated type files, you can uncomment the following line in `.gitignore`:

```gitignore
# Uncomment to ignore auto-generated schema types (developer preference)
# src/**/*.schema.d.ts
```

Alternatively, for a per-developer preference without affecting the team, add this to `.git/info/exclude` (local only, not committed):
...
You can still manually regenerate all types using:
